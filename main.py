...
database = SqliteDatabase('eventDashboard2.db')
...
async def authenticate_user(req: AuthRequest):
    try:
        ...
        user_from_db = User.select().where(User.email == decoded_user.email).first()
        if user_from_db is None:
            user = User(
                first_name=decoded_user.first_name,
                last_name=decoded_user.last_name,
                email=decoded_user.email,
                avatar=decoded_user.avatar
            )
            user.save()
        user_from_db = User.select().where(User.email == decoded_user.email).first()
        return {"token": create_access_token(model_to_dict(user_from_db))}
